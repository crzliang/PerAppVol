...
**阶段 3 —— 产品化** 🚧
- [x] 菜单栏 UI（滑块列表 + App 图标/名字 + 电平表 + 静音 + 全部静音）
- [x] 引擎自动拉起 + TCC 申请 + 失败重试（PID 文件判活，不用 socket 探测）
- [x] UI/引擎双进程分离（UI 崩溃不断音）
- [x] 电平表（`stat` 的峰值保持 pk，AppKit 30Hz 自绘，零 SwiftUI diff）
- [x] 设置持久化 + 自愈（引擎重启/槽位丢失自动恢复）
...
| **P2** | 按 App **独立输出设备**（QQ 走耳机、音乐走音箱） | 引擎已有设备热切换基础 |
| **P2** | 限幅器可调 / soft-clip 后级 | 当前持续过载时压得较狠（-8dB） |
| **P2** | **预设**（会议模式 / 深夜模式） | 一键切换一组音量 |
| **P2** | **错误反馈**（引擎异常只显示红点） | 需要可操作的提示 |
| **P3** | 国际化（现硬编码中文） | 发布到海外才需要 |
| **P3** | 测试（CI 只验证能构建，不验证行为） | 规模大起来再补 |
| **P3** | 图标缓存失效（App 更新后图标可能旧） | 影响小 |
