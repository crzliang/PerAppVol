...
#   make install    安装到 /Applications 并启动（自动迁移旧版 mac-sound-control 的残留）
#   make run        构建并启动（不安装）
#   make migrate    只跑一次旧版迁移（bundle ID / LaunchAgent / socket / 用户设置 / TCC）
#   make clean      清理产物
...
.PHONY: all app tools icon dmg install run autostart autostart-off migrate clean
...
# 先迁移：旧 bundle ID 的 App / LaunchAgent / socket / 用户设置 / TCC 都得先清掉，
# 否则旧 agent 会再拉起一个 App，旧引擎也会占着旧 socket 继续抓音频。
migrate:
	./scripts/migrate-legacy.sh

install: app migrate
	@# 自启的 agent 必须先卸：它 KeepAlive=true，会在 rm -rf / cp 的窗口里把旧 App 拉起来
	launchctl bootout "gui/$$(id -u)/com.perappvol.autostart" 2>/dev/null || true
	...
	@# 原来装了自启就立刻恢复，否则用 open 拉起来
	@if [ -f "$$HOME/Library/LaunchAgents/com.perappvol.autostart.plist" ]; then \
	     launchctl bootstrap "gui/$$(id -u)" "$$HOME/Library/LaunchAgents/com.perappvol.autostart.plist" \
	       && echo "   已按原设置恢复开机自启（无需等下次登录）"; \
	 else \
